...
//#define BEV_EVENT_CONNECTED	0x80	/**< connect operation finished. */
	...
	if (peer)
	{
		...
		{
			std::cout<<"close a client "<< peer->getSocket()<<std::endl;
	...
	struct sockaddr *sock, int socklen, void *arg)
{
	...
	if (bev)
	{
		...
		CRougelikePeer* p = new CRougelikePeer(fd);
		if (p)
		{
			int on = 1;
#ifdef WIN32  
...
bool CNetworkThread::addConnector(const char* ip, short port)
{
	...
	if (p_connect_event)
	{
		...
		if (r == 0)
		{
			CRougelikeTestClient * p = new CRougelikeTestClient((int)event_get_fd(&(p_connect_event->ev_read)));
			if (p)
			{

...
{
	{
		...
		SObjectAction a = { 0 };
		a.action = E_ObjectAction_Move_Start;
		a.objID = getSocket();
		a.minfo.pos_x = 1;
		a.minfo.pos_y = -1;
		a.minfo.dir_x = (float)g_pSceneMgr->getRandomDir()/100;
		a.minfo.dir_y = (float)g_pSceneMgr->getRandomDir() /100;
		SObjectsActionAsk ask = { 0 };
		ask.oas.count = 1;
		ask.oas.a[0] = a;

		EASYSEND(C_2_S_Objects_Action_Ask, SObjectsActionAsk, ask, this);

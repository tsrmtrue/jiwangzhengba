...
public:
	...
	{
		m_cond.waitFor(m_waitMilliSecond);
	}
	virtual bool stop()
	{
		m_bStop = true;
	...
	bool setWaitMillisecond(int ms) 
	{
		m_waitMilliSecond = ms; 
		return true; 
	}
	...
	bool initThread() override
	{
		...
		if (m_pBase != NULL)
		{
			if (m_bNeedTimer)
			{
				...
				m_pEvent = event_new(m_pBase,-1, EV_PERSIST, CThreadLE::cb, this);
				return m_pEvent
